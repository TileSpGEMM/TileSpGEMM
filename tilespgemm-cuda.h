                                                              ...
                                                              int numblkC)
{
    ...
    for (int tilei = tile_start; tilei < tile_end; tilei++)
    {
        ...
        if (tilei < numblkC && nnzcnt_sum)
        {
            ...
            d_blkcsr_Ptr_C[tilei * BLOCK_SIZE + halfwarp_lane_id] = nnzcnt_scan; // - nnzcnt;

            d_blkmaskC[tilei * BLOCK_SIZE + halfwarp_lane_id] = maskc; //s_maskc_local[halfwarp_lane_id];
...
#pragma unroll
        ...
        int nnzcnt_sum = 0;
        d_blkcsr_Ptr_C[tilei * BLOCK_SIZE + 0] = 0;
        d_blkmaskC[tilei * BLOCK_SIZE + 0] = maskc[0];
#pragma unroll
        for (int ci = 1; ci < BLOCK_SIZE; ci++)
        {
            nnzcnt_sum += __popc(maskc[ci - 1]);
            d_blkcsr_Ptr_C[tilei * BLOCK_SIZE + ci] = nnzcnt_sum;
            d_blkmaskC[tilei * BLOCK_SIZE + ci] = maskc[ci];
        }
                                                      ...
                                                      int *d_spec_intersection_posb)
{
    ...
    if (lane_id < BLOCK_SIZE)
        s_blkcsr_Ptr_C_local[lane_id] = d_blkcsr_Ptr_C[tilei * BLOCK_SIZE + lane_id];

    unsigned int maskc = lane_id < BLOCK_SIZE ? d_blkmaskC[tilei * BLOCK_SIZE + lane_id] : 0; //s_maskc_local[lane_id];
...
#pragma unroll
    ...
    {
        unsigned short maskc = d_blkmaskC[tilei * BLOCK_SIZE + lane_id]; //s_maskc_local[lane_id];
        int cnt = 0;
        int blknnzcstart = d_blkcsr_Ptr_C[tilei * BLOCK_SIZE + lane_id];
#pragma unroll 16
        for (int i = 0; i < BLOCK_SIZE; i++)
        {
            int idx = ((maskc >> BLOCK_SIZE - i - 1) & 0x1) == 1 ? i : -1;
