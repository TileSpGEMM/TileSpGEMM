...
//--------------------------------------------------------------------------
    ...
    *gflops_segmerge = 2 * (double)nnzCub / (1e6*time_cuda_spgemm);
    ...
    if (check_result)
    {
        if (*nnzC <=0){            printf("cuSPARSE failed!\n");
}
else{
        printf("\nValidating results...\n");
        if (*nnzC != nnzC_golden)
        ...
        {
            //printf("i = %i, h_csrRowPtrC = %i, h_csrRowPtrC_golden = %i\n",
            //       i, h_csrRowPtrC[i], h_csrRowPtrC_golden[i]);
            if (h_csrRowPtrC[i] != h_csrRowPtrC_golden[i])
            {
                printf("h_csrRowPtrC[i] = %i,  h_csrRowPtrC_golden[i] = %i\n",h_csrRowPtrC[i] ,h_csrRowPtrC_golden[i]);
                errcounter++;
            }
        }
        if (errcounter != 0)
