...
double bytes = (matrixA->tilem + 1) * sizeof(int) + matrixA->numtile * sizeof(int) + (matrixA->numtile + 1) *sizeof(int) +
...
printf("tile space overhead = %.2f MB\n", mem);
    ...
    unsigned int *blk_intersec_bitmask_A = (unsigned int *)malloc(matrixA->tilem * blk_intersec_bitmask_len * sizeof(unsigned int));
    memset(blk_intersec_bitmask_A, 0, matrixA->tilem * blk_intersec_bitmask_len * sizeof(unsigned int));
    for (int i = 0; i < matrixA->tilem; i++)
    {
        for (int j = matrixA->tile_ptr[i]; j < matrixA->tile_ptr[i + 1]; j++)
        {
            ...
            bitmask <<= (31 - (idx % 32));
            blk_intersec_bitmask_A[i * blk_intersec_bitmask_len + idx / 32] |= bitmask;
        }
    }

    unsigned int *blk_intersec_bitmask_B = (unsigned int *)malloc(matrixB->tilen * blk_intersec_bitmask_len * sizeof(unsigned int));
    memset(blk_intersec_bitmask_B, 0, matrixB->tilen * blk_intersec_bitmask_len * sizeof(unsigned int));
    for (int i = 0; i < matrixB->tilen; i++)
    {
        for (int j = matrixB->csc_tile_ptr[i]; j < matrixB->csc_tile_ptr[i+1]; j++)
        {
            ...
            unsigned int bitmask = 0x1;
            bitmask <<= (31 - (idx % 32));
            blk_intersec_bitmask_B[i * blk_intersec_bitmask_len + idx / 32] |= bitmask;
        }
    }
